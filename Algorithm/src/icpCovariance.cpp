...
//     std::cout << "\nPrinting out [x, y, z, a, b, c] =  " <<x<<"    "<<y<<"    "<<z<<"    "<<a<<"    "<<b<<"    "<<c<<std::endl;
    ...
    size_t size=dataVertex.size.x*dataVertex.size.y;
    
    TrackData *tdata=(TrackData*)trackData.data();    
    /****  Calculating d2J_dX2  ****/
    for (size_t s = 0; s < size; ++s )
    {
      ...
      */
      if(tdata[s].result!=1)
          continue;      
        double pix = dataVertex.data()[s].x;
        double piy = dataVertex.data()[s].y;
        double piz = dataVertex.data()[s].z;
        
        double qix = modelVertex.data()[s].x;
        double qiy = modelVertex.data()[s].y;
        double qiz = modelVertex.data()[s].z;

        double nix = modelNormals.data()[s].x;
        double niy = modelNormals.data()[s].y;
        double niz = modelNormals.data()[s].z;
...
//     std::cout << "\nNumber of Correspondences used for ICP's covariance estimation = " << n << std::endl;
    ...
    for (int k = 0; k < n ; ++k) // row
    {
        ...
        */
        ...
        double pix = dataVertex.data()[k].x;
        double piy = dataVertex.data()[k].y;
        double piz = dataVertex.data()[k].z;

        double qix = modelVertex.data()[k].x;
        double qiy = modelVertex.data()[k].y;
        double qiz = modelVertex.data()[k].z;

        double nix = modelNormals.data()[k].x;
        double niy = modelNormals.data()[k].y;
        double niz = modelNormals.data()[k].z;        
