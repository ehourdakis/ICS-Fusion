				   ...
				   int img_dbl, int descr_width, int descr_hist_bins )
{
	...
	/* check arguments */
	if( ! img )
		fatal_error( "NULL pointer error");
...
IplImage* convert_to_gray32( IplImage* img )
{
	...
	/*** changes by Bernhard Zeisl ***/	
	
	if(img->depth == 8) {
		if(img->nChannels == 1)
			gray8 = (IplImage*) cvClone( img );
		else {
			gray8 = (IplImage*)  cvCreateImage( cvGetSize(img), IPL_DEPTH_8U, 1 );
			cvCvtColor( img, gray8, CV_BGR2GRAY );
		}
		cvConvertScale( gray8, gray32, 1.0 / 255.0, 0 );
	}
	else {
		cvConvertScale( img, gray32, 1, 0 );
	}
...
@param contr_thr low threshold on contrast
