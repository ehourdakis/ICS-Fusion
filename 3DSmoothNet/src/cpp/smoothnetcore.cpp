...
//             writerTemp = descriptor[i][j];
            ...
            int idx = i + j*max_x;
            //outFile<<descriptor[idx]<<",";
            fprintf(fp,"%1.8f,",descriptor[idx]);
        }
        int idx=i + (max_y-1)*max_x;
        //outFile<<descriptor[idx]<<"\n";
        fprintf(fp,"%1.8f\n",descriptor[idx]);
    }
    //outFile.close();
                     ...
                     std::vector<std::vector <float>>& neighbors_smoothing_distance)
{
    ...
    for (i = 0; i < pts_size; i++)
    {
        ...
        if (kdtree.radiusSearch(query_point, sup_radius, point_idx, point_dst) > 10)//only if there are more than 10 points in the local surface
        {
            ...
            //point_dst is the square distance. find the root
            ...
            //subs two iterators to get their distance in elements, thus idx 
                              ...
                              float *DIMATCH_Descriptor)
{
    ...
    for (int i = 0; i < pts_size; i++)
    {
        ...
        if (sphere_neighbors->points.size() != 0)
        {
            ...
            {
                //int idx = i + d*pts_size;
                int idx = d + i*descriptor[tid].size();
                DIMATCH_Descriptor[idx] = descriptor[tid][d];
            }
        }
        else
        {
